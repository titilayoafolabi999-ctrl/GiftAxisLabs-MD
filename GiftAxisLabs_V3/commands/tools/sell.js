...
module.exports = {
    ...
    async execute(sock, m, args, reply) {
        if (!args.length) return reply("Usage: .sell <item name>\nSee your items: .inventory");
        const db = await getDB(); const userId = m.key.participant || m.key.remoteJid;
        const user = getUser(db, userId);
        const query = args.join(" ").toLowerCase();
        const itemDef = ITEMS.find(i => i.name.toLowerCase().includes(query) || i.id === query);
        if (!itemDef) return reply("❌ Unknown item. Check .inventory for your items.");
        const idx = user.inventory.indexOf(itemDef.id);
        if (idx === -1) return reply(`❌ You don't have ${itemDef.name}.`);
        const sellPrice = Math.floor(itemDef.price * 0.6);
        user.inventory.splice(idx, 1);
        user.balance += sellPrice;
        await saveDB(db);
        reply(`💰 Sold ${EMOJIS[itemDef.id]} *${itemDef.name}* for *$${sellPrice}*\n💳 Balance: $${user.balance}` + config.footer);
    }
};